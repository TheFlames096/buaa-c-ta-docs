...
格式说明符必须与变量类型对应。类型不对应时，程序可能读错数据或输出错误结果。特别注意：\verb|double| 在 \verb|scanf| 中使用 \verb|%lf|，在 \verb|printf| 中使用 \verb|%f|。

\subsection{scanf 中为什么通常要写取地址符}

程序运行时，变量暂时存放在计算机的\term{内存}中。\verb|scanf| 要把读到的数据放进变量，所以必须知道变量在内存中的位置。变量名前的 \verb|&| 叫作\term{取地址符}，表示“这个变量的位置”。
...
第二段代码没有把变量 \verb|n| 的位置交给 \verb|scanf|。程序可能崩溃，也可能得到无法预测的结果。\verb|printf| 只读取变量中已有的值，不会向变量里存入新数据，因此输出普通变量时通常不写 \verb|&|。
...
\texttt{\&\&} & 并且 & \texttt{a > 0 \&\& b > 0} 要求两边都成立 \\
\texttt{||} & 或者 & \texttt{a == 0 || b == 0} 只要求一边成立 \\
\bottomrule
\end{tabularx}
\end{center}
...
\subsection{并且和或者怎样判断}

\verb|&&| 表示“并且”，左右两边都成立，整个条件才成立。\verb+||+ 表示“或者”，左右两边只要有一边成立，整个条件就成立。
...
条件较长时，先把题目的句子拆开，再用括号写清组合关系。例如，“年份能被 \verb|400| 整除，或者能被 \verb|4| 整除但不能被 \verb|100| 整除”写成：
